...
as $main$
begin 
	instert into actor(first_name, last_name, last_update)
	values(firstName, latName, lastUpdate)
end;
$main$
...
as $main$
begin 
	instert into public.mechanic (first_name, last_name)
	values(firstName, lastName)
end;
$main$
language plpgsql;

select addMechanic ('Liam', 'Scharovsky')
select addMechanic ('Lucy', 'Scharovsky')
...
as $main$
begin 
	instert into "public.car" (salesperson_id, customer_id , invoice_id, make, model, year)
	values(_salesperson_id, _customer_id, _invoice_id, make, model, year)
end;
$main$
language plpgsql;

select addCar(null, 1, null, 'Honda', 'Fit', 2015)
